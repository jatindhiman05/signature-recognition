...
S3_BUCKET_NAME = "signature-recognition-assets"
...
def download_demo_dataset():
    """
    Download and extract demo_samples.zip from AWS S3.

    AWS credentials are NOT hardcoded here.
    boto3 automatically uses the EC2 IAM role when deployed
    on EC2.
    """

    # --------------------------------------------------------
    # 1. Check whether dataset already exists locally
    # --------------------------------------------------------
    ...
    try:
        ...
        print(
            f"Downloading demo samples from S3: "
            f"s3://{S3_BUCKET_NAME}/{DEMO_ZIP_NAME}"
        )
        ...
        print(
            f"Successfully downloaded {DEMO_ZIP_NAME} "
            f"from S3."
        )
        ...
        raise RuntimeError(
            f"Could not download {DEMO_ZIP_NAME} "
            f"from S3 bucket '{S3_BUCKET_NAME}'.\n\n"
            f"{type(e).__name__}: {e}"
        ) from e
    ...
    try:
        ...
        raise RuntimeError(
            f"Could not extract {DEMO_ZIP_NAME}.\n\n"
            f"{type(e).__name__}: {e}"
        ) from e
    ...
    if dataset_dir is None:
        ...
        raise RuntimeError(
            "demo_samples.zip was downloaded and extracted, "
            "but the expected folder structure was not found.\n\n"
            "Expected structure:\n\n"
            "demo_samples.zip\n"
            "├── Original/\n"
            "└── Forged/"
...
def get_random_samples(
    ...
    for class_name in [
        ...
        result[class_name] = random.sample(
            images,
            number_of_samples
        )
...
# Use markdown for header instead of HTML
st.markdown("# Signature Recognition")

st.markdown(
    "*Identify whether a handwritten signature is Original or Forged.*"
)
...
with st.expander("ℹ️ About this model"):
    ...
    st.write(
        """
        **Signature Recognition** is an image classification
        system trained on the CEDAR Signature Dataset.
...
st.caption(
    "Don't have an image? Load demo signatures from "
    "the CEDAR dataset and test the model."
)
...
if generate_samples:
    ...
    if dataset_dir is None:
        ...
        dataset_dir = (
            find_existing_demo_dataset()
        )


    # --------------------------------------------------------
    # Dataset doesn't exist → download from S3
    # --------------------------------------------------------
    ...
    if dataset_dir is None:
        ...
        try:
            ...
            with st.spinner(
                "Downloading demo signatures from S3..."
            ):
            ...
            st.error(
                "Could not load demo signatures from S3."
            )
...
with left_column:
    ...
    # Use st.markdown with proper HTML but simplified
    st.markdown("### 📤 Upload your signature")
    st.markdown("*Upload a JPG, JPEG or PNG image.*")
...
with right_column:
    ...
    st.markdown("### 🖼️ Signature Preview")
    st.markdown("*Your selected signature will appear here.*")
    ...
    if (
        ...
        st.info(
            "Upload a signature or select a sample above."
        )
...
if result is not None:
    ...
    if predicted_class == "Unknown":
        ...
        st.warning(
            ...
            f"Confidence: "
            f"**{confidence * 100:.2f}%**\n\n"
            f"The prediction confidence is below "
    ...
    # ========================================================
        ...
        result_column, confidence_column, status_column = (
            st.columns(3)
        )
